...
University department offices field a large volume of routine questions about course offerings, degree requirements, prerequisites, financial aid, and faculty contacts. At NMSU's Department of Computer Science (CS), these questions arrive through email, walk-in visits, and phone calls — channels that impose a time cost on both students and advising staff even when the answers are fully documented in the academic catalog or department website. A conversational AI assistant that can answer accurately and instantly from authoritative sources offers a practical reduction in that burden, provided it can be trusted to retrieve correct, current information and to decline gracefully when it cannot.

This report describes the design, implementation, and evaluation of a prototype retrieval-augmented generation (RAG) chatbot built for the NMSU CS department. The system ingests two primary sources, the NMSU Academic Catalog (PDF) and department web pages, stores them as semantically searchable chunks in a vector database, and uses a large language model (LLM) to compose answers grounded in the retrieved text. A systematic evaluation across 75 representative questions demonstrates the system's strengths and identifies the categories where further work is warranted.
...
**User interface.** A web front end built with Next.js and React provides a conversational chat interface. The interface maintains a scrolling message history, shows a loading indicator while the backend processes the query, and includes a department selector — currently set to the CS department — that allows the architecture to be extended to additional departments without changes to the front end. The interface communicates with the backend via a single REST endpoint (`/chat`). The department identifier is already passed with every request, so extending to multiple departments would require only minor front-end changes: populating the department selector dynamically rather than defaulting to "cs", and making the page title and welcome message respond to the selected department.

**Evaluation harness.** A Python evaluation framework runs the retrieval and generation pipeline against a curated ground-truth dataset, computes deterministic retrieval metrics, invokes an LLM-as-judge for quality scoring, and produces structured output for human review and reporting.
...
The primary authoritative source for course descriptions, degree requirements, prerequisites, minors, and academic policies is the NMSU Academic Catalog (2025–2026 edition, PDF). The catalog PDF spans hundreds of pages; the CS-relevant sections cover undergraduate and graduate course listings, B.S. and M.S. degree requirements, and minor programs.

A custom catalog chunker (`nmsu_catalog_chunker.py`) parses the PDF and produces structured chunks by type: `course_description`, `degree_requirement`, `degree_core_requirement`, `minor_index`, `minor_requirement`, `policy`, and others. Course description chunks carry the course code, title, credit count, prerequisites, and catalog page range. Degree requirement chunks capture program-level requirements including concentration options. Each chunk is assigned a stable, deterministic chunk ID derived from its content type and catalog page so that ground-truth references remain valid across re-ingests.

### 3.2 Department Web Pages

The department website supplements the catalog with current operational information and, in some areas, duplicates it. Faculty directory pages list contact details and brief research expertise descriptions — information that overlaps partially with the catalog's faculty section but is more current and more accessible by name. Information available only on the web includes the three-year course offering rotation, advising contacts, financial aid and assistantship details, and current-semester announcements. The web is also the sole source for the Bachelor of Science in Artificial Intelligence, which was introduced after the current catalog went to press. A focused web crawler (`ingest.py`) retrieves pages from the Advising, CS and Data Analytics department sites, extracts main content, and produces web chunks with URL-based IDs.

The crawler applies a faculty-entry splitter that segments multi-faculty directory pages into per-person chunks, ensuring that retrieval for a faculty-specific query does not surface unrelated colleagues. Chunk IDs for web content are derived from the page URL and a sequential chunk index, making them stable so long as the page URL and content order do not change.

### 3.3 Chunking Design Choices

Chunks in this system are not uniform in length. Rather than splitting text at fixed token or character counts — a common approach in general-purpose RAG frameworks — the pipelines segment content at natural semantic boundaries: one chunk per course description, one per degree program, one per faculty entry, one per policy topic. This content-driven approach means a chunk for a one-paragraph course description may be a few hundred tokens, while a chunk covering a full degree's course requirements may be several times longer. The trade-off is intentional: splitting a degree requirement block at an arbitrary token limit would sever the logical unit a student needs to see whole, producing incomplete and potentially misleading answers. Embedding quality and retrieval precision are better served by coherent semantic units than by size uniformity. A hard upper limit of 8,192 tokens — the maximum accepted by the OpenAI embedding model — is enforced by a preflight check in the catalog ingestion pipeline; any chunk exceeding that limit causes the run to abort before any data is written. In practice, the longest chunk in the current database is approximately 5,100 tokens (a policy section covering doctoral degree requirements), leaving comfortable headroom below the ceiling.
...
- **Heading bleed prevention.** The catalog chunker applies a `NOT_FIRST_WORD` filter to prevent section headings (e.g., "Introduction", "Expertise") from being treated as course or degree content.
- **Calendar table chunking.** The academic calendar is chunked as a unit rather than split by row, since advising queries about deadlines typically need the full calendar context.
...
The top-ranked chunks are assembled into a prompt context block. Each chunk's source citation — catalog year and page range for catalog chunks, URL for web chunks — is included so the LLM can produce accurate citations in the answer. A context-recall metric measures how much of the retrieved content overlaps with the expected chunks for each question, complementing the binary recall-at-k metric.
