...
def parse_weaviate_objects(objects, score_attr: str | None = None) -> List[Dict[str, Any]]:
    results = []
...
def search_chunks(query: str, department_id: str, top_k: int = TOP_K) -> List[Dict[str, Any]]:
    """
    Hybrid retrieval:
      1. semantic search via near_vector
      2. keyword ranking with local BM25 over the semantic candidate pool
      3. score fusion with RRF and metadata boosts
    """
